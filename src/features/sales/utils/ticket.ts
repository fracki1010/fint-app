...
export function generateTicketHTML(data: TicketData): string {
  ...
  const subtotal = showTax ? order.totalAmount / (1 + taxRate / 100) : order.totalAmount;
  const taxAmount = showTax ? order.totalAmount - subtotal : 0;
  ...
  return `
    ...
    <body>
      ...
      <table>
        ...
        ${showTax ? `<tr><td>IVA (${taxRate}%)</td><td class="right">${formatCurrency(taxAmount, currency)}</td></tr>` : ""}
