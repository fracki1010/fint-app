                          ...
                          <AutocompleteItem key={s._id} textValue={s.name}>
                            <div className="flex items-center justify-between gap-3">
                              ...
                              <span className="shrink-0 text-[11px] text-default-400">
                                {s.referenceCost ? formatCompactCurrency(s.referenceCost, currency) : ""}{s.sku ? ` · ${s.sku}` : ""}
                              </span>
                            </div>
                          </AutocompleteItem>
                        )}
...
{(p) => (
                          <AutocompleteItem key={p._id} textValue={p.name}>
                            <div className="flex items-center justify-between gap-3">
                              ...
                              <span className="shrink-0 text-[11px] text-default-400">
                                {p.costPrice ? formatCompactCurrency(p.costPrice, currency) : ""}{p.sku ? ` · ${p.sku}` : ""}{p.barcode ? ` · ${p.barcode}` : ""}
                              </span>
                            </div>
                          </AutocompleteItem>
                        )}
                  ...
                  <label className="block">
                    ...
                    {item.itemKind === "product" && item.presentationName && Number(item.unitCost) > 0 && Number(item.purchaseEquivalentQty) > 0 && (
                      <span className="mt-1 block text-[10px] text-default-400">
                        = {formatCurrency(Number(item.unitCost) / Number(item.purchaseEquivalentQty), currency)} por {products.find((p) => p._id === item.productId)?.unitOfMeasure || "ud. base"}
                      </span>
                    )}
                  </label>
                  <div>
