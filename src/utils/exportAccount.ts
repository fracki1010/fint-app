...
import { ClientAccountEntry, ClientEntryType } from "@/types";
...
export function exportAccountToPDF({
  ...
  doc.text(`${balance > 0 ? "+" : ""}${formatCurrencyForPDF(balance, currency)}`, 18, 56);
  ...
  const tableData = entries.map((entry) => {
    ...
    return [
      ...
      entry.reference || "-",
      entry.paymentMethod || "-",
      formatCurrencyForPDF(signedAmount, currency),
      entry.notes || "",
    ];
  });
